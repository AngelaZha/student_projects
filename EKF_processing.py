...
def estimate(filename):
    ...
    while True:
        ...
        if t == 'XKF2' and m.C == 0:
            ...
            if np.linalg.norm(np.array([m.MN,m.ME,m.MD]))!= 0:
    ...
    print('time step min max is', timestep_min, ' ',timestep_max, '\n average is:', dtaverage)
    # SVAAstate = np.array(output['SVAA.state'])
    # SVABstate = np.array(output['SVAB.state'])
    
    # #Convert delta angle bias from radians to degrees
    # SVAAstate[:,10:13] = SVAAstate[:,10:13]*180/pi
    # #Divide the delta angle bias and delta velocity bias by average timestep
    # SVAAstate[:,10:15] = SVAAstate[:,10:15]/dtaverage
    # SVABstate = SVABstate/dtaverage
    # output['SVAA.state'] = SVAAstate.tolist()
    # output['SVAB.state'] = SVABstate.tolist()
