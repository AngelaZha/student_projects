...
#     return np.random.normal(0,standard_dev)
...
class EqF:
    def __init__(self,gravity, param):
            ...
            self.gpstime = 0
            self.barotime = 0
            # self.P = 0.0001* np.eye(9+6) #process gain noise #0.001 is original, sometimes tuned to 10% (0.0001) of what was before
            #Changed P value to set bias components to zero as per Pieter recommendation 10/13
            ...
            self.P_0 = np.eye(5)
            # self.Riccati = 0.01*np.eye(9+6)
            #Changed initial bias covariance to be much smaller as per Pieter's recommendation 10/13
            ...
            if param == 'false':
                self.Q_posNED = 0.1 *np.eye(3) #~Output covariance
                self.Q_posNE = 0.1 * np.eye(2)
                self.Q_gpsvel = 0.1 * np.eye(3)
                self.Q_posvel = 0.1 * np.eye(6)
                self.Q_baro = 0.1 #separate output covariance to gps's self.Q
                self.Q_mag = 0.1 * np.eye(3)
                
                self.R = 0.1*np.eye(6) #Noise introduced by input
                print("Default param set")
            else:
                self.Q_posNED = np.square(np.eye(3) * np.array([par_dict.get('EK3_POSNE_M_NSE'), par_dict.get('EK3_POSNE_M_NSE'), par_dict.get('EK3_ALT_M_NSE')])) #~Output covariance
    ...
    def construct_B(Ahat):
        ...
        #p_cross = compute_skew_x (p.T.ravel()) #wrote in first version _B_matrix, now corrected all current versions to above
        #v_cross = compute_skew_x (v.T.ravel())
        B[0:3,0:3] = R
        B[3:6,0:3] = -R @ p_cross
        B[6:9,0:3] = -R @ v_cross
        B[6:9,3:6] = R
    ...
    def stateEstimate(self): #right action
    ...
    @staticmethod
    def construct_inputmat(B):
        inputmat = np.zeros((15,6))
        inputmat[0:9,0:6] = B
        return inputmat
    
    @staticmethod
    def outputmat_gps_NE(C_gps):
        outputmat = np.zeros((2,15))
        outputmat[0:2,0:9] = C_gps
        return outputmat
    
    @staticmethod
    def outputmat_gps_NED(C_gps):
        outputmat = np.zeros((3,15))
        outputmat[0:3,0:9] = C_gps
        return outputmat
    
    @staticmethod
    def outputmat_gps_posvel(C_gps):
        outputmat = np.zeros((6,15))
        outputmat[0:6,0:9] = C_gps
        return outputmat
    
    @staticmethod
    def outputmat_baro(C_baro):
        outputmat = np.zeros((1,15))
        outputmat[:,0:9] = C_baro
        return outputmat
    
    @staticmethod
    def outputmat_mag(C_mag):
        outputmat = np.zeros((3,15)) #same format as gps_NED outputmat
        outputmat[0:3,0:9] = C_mag
        return outputmat
    
    def Ahat_wo_innov(self,gyro,accel,currentimutime):
        ...
        #Riccati_dot = state_mat @ self.Riccati + self.Riccati @ state_mat.T + input_mat @ self.R @ input_mat.T + self.P
        
        #Modify Riccati term so ensure positive sem matrix
        #self.Riccati = self.Riccati + timestep*Riccati_dot
    ...
    def gpscorrect(self,x_p, currentgpstime):
        ...
        #Discrete time update (update Riccati)
        ...
        Delta = self.Riccati @ output_mat.T @ np.linalg.inv(S) @(y-p_Ahat[0:2]) #delete the altitude component
        ...
        # #Uncomment this section if using GPS NED
    ...
    def gpsvelcorrect(self,vel,currentgpstime):
        timestep = currentgpstime - self.gpstime
        self.gpstime = currentgpstime
        R_Ahat, p_Ahat, v_Ahat = SE23_from_mat(self.Ahat)
        ...
        self.Ahat = expm(vect2se23(Delta[0:9,:])) @ self.Ahat
        self.bhat = self.bhat + Delta[9:15,:]
        #self.velNIS = ((v-v_Ahat).T @ np.linalg.inv(S) @ (v-v_Ahat)).item()
        
    ## Uncomment the following only if want to correct using GPS pos and vel and the same time
    # def gpscorrect(self,x_p, vel, currentgpstime):
    #     timestep = currentgpstime - self.gpstime
    #     self.gpstime = currentgpstime
    #     R_Ahat, p_Ahat, v_Ahat = SE23_from_mat(self.Ahat)
    ...
    #     #Uncomment this section if using GPS NED
    #     pos_vel = np.array([[x_p.x],[x_p.y],[x_p.z],[vel.x],[vel.y],[vel.z]])
    #     p_v_Ahat = np.vstack((p_Ahat, v_Ahat))
    #     C = EqF.construct_C_GPS_posvel(p_Ahat,v_Ahat)
    #     output_mat = EqF.outputmat_gps_posvel(C)
    #     S = output_mat @ self.Riccati @ output_mat.T + self.Q_posvel
    #     K = self.Riccati @ output_mat.T @ np.linalg.inv(S)
    #     Delta = self.Riccati @ output_mat.T @ np.linalg.inv(S) @(pos_vel- p_v_Ahat)
              
    #     self.Riccati = (self.identity - K @ output_mat) @ self.Riccati
    #     self.Ahat = expm(vect2se23(Delta[0:9,:])) @ self.Ahat
    #     self.bhat = self.bhat + Delta[9:15,:]
    #     self.velNIS = ((pos_vel- p_v_Ahat).T @ np.linalg.inv(S) @ (pos_vel- p_v_Ahat)).item()
        
    def barocorrect(self, alt):
        R_Ahat, p_Ahat, v_Ahat = SE23_from_mat(self.Ahat)
        ...
        self.Riccati = (self.identity - K @ output_mat) @ self.Riccati
        # val, vect = np.linalg.eig(self.Riccati)
        # assert all(val>0) == True,"Eigen value of Riccati term is negative"
        self.Ahat = expm(vect2se23(Delta[0:9,:])) @ self.Ahat
        self.bhat = self.bhat + Delta[9:15,:]    
        self.altNIS = ((alt-(-p_Ahat[2])).T @ np.linalg.inv(S) @ (alt-(-p_Ahat[2]))).item()
        
    def magcorrect(self, latlon, field):
        R_Ahat, p_Ahat, v_Ahat = SE23_from_mat(self.Ahat)
        ...
        #Normalise mag readings
        #29/09 commented out normalisation because need to adjust this with proper mag noise
        norm_mag = mag#/np.linalg.norm(mag)
        ...
        #29/09 commented out e1_earth normalisation because need to adjust this with proper mag noise
        #e1_earth = e1_earth/np.linalg.norm(e1_earth)
        #e1 using WMM is approx array([[0.40144449],[0.09011895],[-0.91143891]])
        # e1 = np.array([[0.395346176504478 ],[0.11012878308931125],[-0.9114719248336718]])#Got from average nolag-6 inertial mag
        # e1 = e1/np.linalg.norm(e1) #to make sure mag of e1 is 1
        # e1_SITL = np.array([[0.40144442021641813],[0.0901189618129463],[-0.911438944855255]])
        # e1_SITL = e1_SITL/np.linalg.norm(e1_SITL)
        ...
        self.Riccati = (self.identity - K @ output_mat) @ self.Riccati
        ## Sanity check of Riccati matrix is semi-positive definite
        # val, vect = np.linalg.eig(self.Riccati)
        # assert all(val>0) == True,"Eigen value of Riccati term is negative"
        self.Ahat = expm(vect2se23(Delta[0:9,:])) @ self.Ahat
        self.bhat = self.bhat + Delta[9:15,:]
        # self.magNIS = ((norm_mag - R_Ahat.T @ e1_earth).T @ np.linalg.inv(S) @ (norm_mag - R_Ahat.T @ e1_earth)).item()
...
class Estimator(object):
    ...
    def update_GPS(self, position, velocity,currenttime):
        ...
        ## Only uncomment the following if want to update GPS pos and vel at the same time
        # self.eqf.gpscorrect(x_p,velocity,currenttime)
...
def estimate(filename, param):
    ...
    output = { 'EqF.Roll' : [],
                ...
                'EqF.GPSrotT':[],
                'Riccati.eig':[],
                #Check eigenvectors and which is most uncertain, can be deleted later
                'eigen.time':[],
                'eigenvect':[],
                'EqF.origin':[],
                'EqF.gpsinnovsquared':[]
    ...
    #bias_dang = np.random.randn(3,1) * 0.01
    #bias_dvel = np.random.randn(3,1) * 0.05
    # bias_dang = np.array([[-1.77105251e-05],[ 7.32672652e-05],[-1.33407913e-05]])
    # bias_dvel = np.random.randn(3,1) * 0.00
    #bias_dang = np.array([[-0.00727805],[0.00063003],[0.00312882]])
    #bias_dvel = np.array([[0.05221103],[0.01816034],[0.01040202]])
    ...
    while True:
        ...
        if t == 'RGPJ' and m.I == 0 and RGPI.Stat >= 3:
            ...
            #Output eigen value of the riccati
        ...
        if t == 'RMGI' and m.I == 0 and m.H == 1:
            ...
            # output['EqF.magNIS'].append(est.eqf.magNIS)
